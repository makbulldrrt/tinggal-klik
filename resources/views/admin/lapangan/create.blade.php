<x-app-layout>
    ...
    <style>
        .form-wrap{max-width:680px;margin:36px auto;padding:0 24px;font-family:system-ui,Inter,sans-serif;color:#1d1d1f}
        .form-card{background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:36px 40px}
        .form-title{font-size:20px;font-weight:700;letter-spacing:-.2px;margin-bottom:28px}
        .form-group{margin-bottom:22px}
        label{display:block;font-size:15px;font-weight:500;color:#1d1d1f;margin-bottom:7px}
        .form-control{width:100%;font-size:16px;color:#1d1d1f;background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:11px 14px;outline:none;transition:border-color .15s,box-shadow .15s;box-sizing:border-box;font-family:inherit}
        ...
        .form-actions{display:flex;gap:12px;align-items:center;margin-top:32px;padding-top:24px;border-top:1px solid #f0f0f0}
        .btn-primary{display:inline-flex;align-items:center;background:#0066cc;color:#fff;font-size:15px;font-weight:500;padding:10px 26px;border-radius:999px;border:none;cursor:pointer;transition:background .15s;font-family:inherit}
        .btn-primary:hover{background:#0055aa}
        .btn-cancel{display:inline-flex;align-items:center;background:transparent;color:#0066cc;font-size:15px;font-weight:500;padding:10px 22px;border-radius:999px;border:1px solid #0066cc;text-decoration:none;transition:background .15s,color .15s;font-family:inherit}
    ...
    <div class="form-wrap">
        <div class="form-card">
            <div class="form-title">Tambah Lapangan Baru</div>
            ...
            <form action="{{ route('admin.lapangan.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select id="jenis_olahraga" name="jenis_olahraga"
                        ...
                        <option value="Tenis" {{ old('jenis_olahraga') === 'Tenis' ? 'selected' : '' }}>Tenis</option>
                        <option value="Voli" {{ old('jenis_olahraga') === 'Voli' ? 'selected' : '' }}>Voli</option>
                        <option value="Lainnya" {{ old('jenis_olahraga') === 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="deskripsi" name="deskripsi"
                        ...
                        placeholder="Tuliskan deskripsi singkat tentang lapangan...">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="error-msg">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <p class="hint">Format: JPG, JPEG, PNG, WEBP. Maks 2MB.</p>
                    @error('foto_lapangan')
                        <p class="error-msg">{{ $message }}</p>
                    @enderror
                </div>
