...
<head>
    ...
    <meta name="description" content="Edit Lapangan — Tinggal Klik Owner">
...
<div class="flex-1 flex flex-col overflow-hidden">
    <header class="bg-white border-b border-slate-100 px-8 py-4 flex items-center gap-4 flex-shrink-0">
        ...
        <div>
            <h1 class="text-lg font-semibold text-slate-900">Edit Lapangan</h1>
            <p class="text-xs text-slate-400 mt-0.5">{{ $lapangan->nama_lapangan }}</p>
        </div>
    </header>
    ...
    <main class="flex-1 overflow-y-auto px-8 py-7">
        <div class="max-w-2xl">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-5 flex items-center gap-4">
                <img src="{{ asset('storage/' . $lapangan->foto_lapangan) }}" alt="{{ $lapangan->nama_lapangan }}" class="w-20 h-20 rounded-xl object-cover">
                <div>
            ...
            <form method="POST" action="{{ route('owner.lapangan.update', $lapangan->id) }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nama_lapangan" name="nama_lapangan" value="{{ old('nama_lapangan', $lapangan->nama_lapangan) }}" class="form-input" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="jenis_olahraga" name="jenis_olahraga" class="form-input">
                            ...
                            <option value="{{ $jenis }}" {{ old('jenis_olahraga', $lapangan->jenis_olahraga) === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="harga_per_jam" class="form-label">Harga per Jam (Rp)</label>
                        <input type="number" id="harga_per_jam" name="harga_per_jam" value="{{ old('harga_per_jam', $lapangan->harga_per_jam) }}" class="form-input" min="1" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="form-input resize-none" required>{{ old('deskripsi', $lapangan->deskripsi) }}</textarea>
                </div>

                <div>
                    <label for="foto_lapangan" class="form-label">Ganti Foto Lapangan</label>
                ...
                <div class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
                    ...
                    <button type="submit" id="btn-update-lapangan" class="px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 transition-all shadow-sm">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
